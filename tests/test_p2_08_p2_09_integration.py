...
def test_desktop_uses_shared_polling_and_shadow_dashboard_without_execution_controls():
    ...
    primary_block = navigation.split("export const PRIMARY_NAVIGATION", 1)[1].split("export const ADVANCED_NAVIGATION", 1)[0]
    assert primary_block.count('group: "observe"') == 5
    ...
    assert 'page === "memory"' in app_pages
    ...
    assert "不把运维工作冒充成主人待办" in attention
