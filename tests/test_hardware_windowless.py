...
class _RejectingRunner:
    ...
    def command(self, args: list[str], *, timeout: float = 3.0):
        ...
        raise AssertionError(f"static hardware detection must not spawn a process: {args}")


def test_static_windows_hardware_detection_does_not_spawn_processes() -> None:
...
def test_hardware_detector_contains_no_powershell_probe() -> None:
