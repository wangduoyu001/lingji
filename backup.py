...
def backup_code():
    ...
    dest = BACKUP_ROOT / 'code' / f'src_{ts}'
    dest.mkdir(parents=True, exist_ok=True)
    for f in ['main.py','run_service.py','start_lingji.py','start_lingji.bat','requirements.txt','.gitignore']:
        src = BASE_DIR / f
        if src.exists():
...
def backup_data():
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    dest = BACKUP_ROOT / 'data' / f'data_{ts}'
    dest.mkdir(parents=True, exist_ok=True)
...
def backup_vault():
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    dest = BACKUP_ROOT / 'snapshots' / f'vault_{ts}'
    dest.mkdir(parents=True, exist_ok=True)
    src = BASE_DIR / 'vault'
    if src.exists():
...
def cleanup_old(keep_days=14):
    ...
    for subdir in BACKUP_ROOT.rglob('*'):
...
def full_backup():
    manifest = {'timestamp': datetime.now().isoformat(), 'paths': {'code': backup_code(), 'data': backup_data(), 'vault': backup_vault()}}
