...
4. 任何开发、优化、修复、发布或验收任务都必须阅读 `docs/ACCEPTANCE/README.md`；执行真机验收时继续读取 `docs/ACCEPTANCE/CODEX_ACCEPTANCE_INSTRUCTIONS.md` 和 `CHANGE_ACCEPTANCE_LOG.md` 最新相关条目。
5. 阅读直接受影响的源代码、直接调用方和对应测试。
6. 只有当任务改变架构边界、数据权威、端口、主线职责或兼容策略时，才阅读 `docs/ARCHITECTURE.md`。
7. 只有当任务涉及治理、发布、跨模块测试或文档职责时，才阅读 `docs/DEVELOPMENT_RULES.md` 的对应章节。
8. 历史模块计划和测试报告只在核对历史结论、回归边界或兼容承诺时读取。
...
4. 给出有边界的落地计划，再修改代码。
5. 使用最少、清晰、可维护的代码完成任务，优先扩展现有模块。
6. 不为“更整齐”做无关的大规模重构，不建立第二套数据库、检索器、队列、API、UI 或配置中心。
7. 每个功能或较大代码修改必须有对应测试，并在现有权威文档或 `docs/TEST_REPORTS/` 留下 Markdown 记录。
8. 代码、运行时、Desktop、连接器、数据链路、脚本、依赖或发布流程变化时，必须在同一 PR 同步更新 `docs/ACCEPTANCE/`；未同步不得宣布完成或请求合并。
...
- 验收默认直接覆盖安装，不卸载和删除主人数据；临时 Artifact、日志、截图、fixture 和配置副本在报告提交后清理。
...
- `docs/ACCEPTANCE/`：当前统一验收标准、Codex 执行指令、每次变更的验收增量和报告模板。
