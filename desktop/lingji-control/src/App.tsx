...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark">灵</div><div><strong>灵机</strong><span>本地控制中心</span></div></div><nav>{NAVIGATION.map((item) => <button key={item.id} className={page === item.id ? "nav-item active" : "nav-item"} onClick={() => setPage(item.id)}><span>{item.label}</span><small>{item.hint}</small></button>)}</nav><div className="sidebar-footer"><span className={connection.connected ? "status-dot online" : "status-dot"} />{connection.connected ? "本机服务已连接" : "本机服务未连接"}</div></aside>
    <main className="main-area"><header className="topbar"><div><h1>{current.label}</h1><p>{current.hint}</p></div><div className="connection-controls"><input value={connection.baseUrl} onChange={(event) => connection.setBaseUrl(event.target.value)} aria-label="API 地址" /><input value={connection.token} onChange={(event) => connection.setToken(event.target.value)} type="password" placeholder="控制令牌" aria-label="控制令牌" /><button className="button secondary" onClick={() => void connection.connect()}>连接</button></div></header>
      {connection.error && <Notice kind="error">{connection.error}</Notice>}{!connection.connected && <Notice kind="warning">先启动 <code>python run_control_api.py</code>。浏览器开发模式需填写 <code>storage/control_api_token</code>。</Notice>}
