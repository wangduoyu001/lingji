  ...
  return (
    <div className="stack">
      <div className="toolbar">
        <button className="button secondary" onClick={() => fetchStatus()}>刷新</button>
        <span style={{ marginLeft: 8, opacity: 0.7 }}>状态: {data?.processing_status || "未知"}</span>
        <span style={{ marginLeft: 12, opacity: 0.7 }}>系统: {data?.system_status || "未知"}</span>
      </div>
      <div className="metric-grid">
        <Metric title="记忆数量" value={String(data?.memory_count ?? "-")} detail={`向量 ${data?.vector_count || 0}`} />
