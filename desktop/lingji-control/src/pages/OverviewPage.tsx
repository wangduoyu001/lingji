...
function eventText(event: OverviewEvent): { title: string; detail: string; tone: FlowTone } {
  ...
  if (type === "capture_duplicate") {
    return { title: "发现重复资料，已自动跳过", detail: "没有创建第二份任务或重复记忆。", tone: "done" };
  }
  if (type === "capture_job_retried") {
    return { title: "失败任务已自动重新排队", detail: "保留原失败记录，并继续按重试策略处理。", tone: "active" };
  }
  if (type === "capture_job_cancelled") {
    return { title: "任务已结束", detail: "任务已从处理队列移除，历史记录仍保留。", tone: "waiting" };
  }
  if (type === "capture_paused") {
    return { title: "资料收纳已暂停", detail: "暂停期间不会接收新的采集任务。", tone: "issue" };
  }
  if (type === "capture_resumed") {
    return { title: "资料收纳已恢复", detail: "新的资料会继续自动进入处理流程。", tone: "done" };
  }
  if (type === "autopilot_repair") {
    return { title: "已自动修复一个运行问题", detail: "修复后已经重新检查，不需要手动重复操作。", tone: "done" };
  }
  if (type === "autopilot_cycle_failed") {
    return { title: "自动巡检暂时失败", detail: "已保留错误并等待下一轮自动重试。", tone: "issue" };
  }
  if (type === "real_environment_acceptance") {
    return { title: "已完成一次环境检查", detail: "检查结果已保存，可在高级记录中追溯。", tone: "done" };
  }
  return {
...
export default function OverviewPage({
  ...
  const focusTitle = currentFocus?.title
    ?? (latestAutomaticAction?.title ? `刚自动处理 · ${String(latestAutomaticAction.title)}` : "后台持续巡检");
  const focusDetail = currentFocus?.detail
    ?? (latestAutomaticAction?.verified === true ? "这一步已经自动复验。" : "没有需要你手动刷新的状态。 ");

  const workflowStages = useMemo<FlowStage[]>(() => {
    const hasMaterial = (documents ?? 0) > 0 || recentQueue.length > 0;
    const vectorState = String(vector.state ?? "unknown").toLowerCase();
    const vectorIssue = !["healthy", "ready", "available", "unknown"].includes(vectorState);
    return [
      {
        id: "discover",
        index: "01",
        title: "发现来源",
        state: "持续扫描",
        detail: "识别本机 AI 工具和可导入资料，只读取元数据。",
        tone: "active",
      },
      {
        id: "intake",
        index: "02",
        title: "收纳",
        state: hasMaterial ? `已收纳 ${quantity(documents)} 份` : "等待新资料",
        detail: chunks === null ? "资料进入后自动去重并排队。" : `当前形成 ${quantity(chunks)} 个可检索片段。`,
        tone: hasMaterial ? "done" : "waiting",
      },
      {
        id: "parse",
        index: "03",
        title: "解析",
        state: runningTasks > 0 ? `${runningTasks} 项处理中` : failedTasks > 0 ? `${failedTasks} 项失败` : completedTasks > 0 ? "最近处理完成" : "等待输入",
        detail: runningTasks > 0 ? "读取、提取、结构化按队列连续执行。" : failedTasks > 0 ? "失败原因已保留，自动重试到上限后停止。" : "有新资料时自动开始。",
        tone: runningTasks > 0 ? "active" : failedTasks > 0 ? "issue" : completedTasks > 0 ? "done" : "waiting",
      },
      {
        id: "candidate",
        index: "04",
        title: "候选",
        state: pendingReviewCount > 0 ? `${pendingReviewCount} 条待审核` : "没有待审核候选",
        detail: "提取结果先成为候选，不直接写进永久记忆。",
        tone: pendingReviewCount > 0 ? "owner" : "done",
      },
      {
        id: "confirm",
        index: "05",
        title: "确认",
        state: ownerDecisionCount > 0 ? `${ownerDecisionCount} 项等你决定` : "当前无需确认",
        detail: ownerDecisionCount > 0 ? "只有权限、永久记忆和不可逆操作会停在这里。" : "安全的后台步骤不会打断你。",
        tone: ownerDecisionCount > 0 ? "owner" : "done",
      },
      {
        id: "index",
        index: "06",
        title: "索引",
        state: vectorIssue ? "语义索引降级" : coveragePercent === null ? "正在确认覆盖" : `覆盖 ${coveragePercent}%`,
        detail: vectorIssue ? "全文检索继续可用，系统不会擅自重建索引。" : "新增内容会自动更新可重建索引。",
        tone: vectorIssue ? "issue" : coveragePercent !== null ? "done" : "active",
      },
      {
        id: "retrieve",
        index: "07",
        title: "取回",
        state: coveragePercent === null ? "等待索引" : "可以开始检索",
        detail: String(retrieval.precision_message ?? "没有验证样本时不宣称准确率。"),
        tone: coveragePercent === null ? "waiting" : "done",
      },
    ];
  }, [coveragePercent, completedTasks, documents, failedTasks, ownerDecisionCount, pendingReviewCount, recentQueue.length, retrieval.precision_message, runningTasks, vector.state, chunks]);

  const visibleEvents = overviewEvents.slice(0, 5);
  const memoryHeadline = documents === null
    ? "正在确认记忆状态"
    : documents === 0
      ? "还没有正式收纳的资料"
      : `已收纳 ${quantity(documents)} 份资料`;
  const memoryDetail = coveragePercent === null
    ? `${quantity(chunks)} 个片段 · 索引覆盖正在建立`
    : `${quantity(chunks)} 个片段 · 当前索引覆盖 ${coveragePercent}%`;
    ...
    <div className="stack overview-page observation-page owner-autopilot-home owner-home-v2">
      <section className={`autopilot-command-center ${ownerDecisionCount > 0 ? "needs-owner" : systemIssueCount > 0 ? "has-issue" : "is-calm"}`}>
        <div className="autopilot-command-copy">
          ...
          {ownerDecisionCount > 0 && (
            <button className="button primary" onClick={() => onNavigate("attention")}>只看需要我决定的事</button>
          )}
          {ownerDecisionCount === 0 && systemIssueCount > 0 && (
            <button className="text-button" onClick={() => onNavigate("activity")}>查看自动处理记录</button>
          )}
        </div>
      ...
      <section className="autopilot-flow-surface" aria-label="自动工作流">
        <div className="autopilot-section-heading">
          <div>
            <span className="desktop-eyebrow">自动工作流</span>
            <h2>从发现到可取回，系统自己往下走</h2>
          </div>
      ...
      <div className="autopilot-home-grid">
        <section className="autopilot-event-stream" aria-label="最近自动处理记录">
          <div className="autopilot-section-heading compact">
            <div>
              <span className="desktop-eyebrow">最近自动完成</span>
              <h2>不是“在线”，而是真的做过什么</h2>
            </div>
          </div>
          {visibleEvents.length > 0 ? (
            ...
            <div className="autopilot-empty-stream">
              <strong>还没有新的处理记录</strong>
              <p>后台巡检和来源扫描仍在继续；有实际动作后这里会留下记录。</p>
            </div>
          )}
        </section>
        ...
        <section className="memory-progress-v2" aria-label="记忆进度">
          ...
          <p>{memoryDetail}</p>
          <div className="memory-progress-v2-meter">
            <span style={{ width: `${coveragePercent ?? 0}%` }} />
          </div>
