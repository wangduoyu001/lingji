...
export default function OverviewPage({ data, api, active, onNavigate }: { data: Row | null; api: LingJiApi; active: boolean; onNavigate: (page: PageId) => void }) {
  ...
  return <div className="stack overview-page observation-page">
    ...
    <section className="overview-section overview-next-step">
      <div className="overview-section-heading"><div><h3>下一步</h3><p className="overview-section-lede">这里始终只给你一个清楚的下一步。</p></div>{pendingActions.length > 0 && !pendingUnavailable && <button className="button primary" onClick={() => onNavigate("attention")}>去处理</button>}</div>
      <p className="overview-readable-line">{pendingUnavailable ? "灵机正在确认是否需要你处理，请稍后再看。" : pendingActions.length ? `需要你处理：${display(pendingActions[0]?.description, "一项待确认事项")}` : "灵机会继续自动检查，你现在不用做任何事。"}</p>
    </section>
