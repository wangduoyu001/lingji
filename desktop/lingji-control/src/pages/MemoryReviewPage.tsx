...
export default function MemoryReviewPage({ api, active }: PageProps) {
  ...
  const openCandidate = async (row: MemoryCandidate) => {
    ...
    try { const detail = await client.candidate(row.memory_id); setSelected(detail); setEditContent(detail.content ?? ""); setIntegrity(await client.integrity(row.memory_id).catch(() => null)); }
    catch (reason) { if (reason instanceof ApiError) setError(reason); } finally { setBusy(""); }
  };

  const approve = async (edited: boolean) => {
    if (!selected?.content_hash || busy) return;
    const prompt = "此操作会把候选记忆加入长期记忆，并在后续 Codex 对话中参与取回。";
    if (!window.confirm(prompt)) return;
    setBusy(edited ? "edit-approve" : "approve");
    try {
      if (edited) await client.editApprove(selected.memory_id, selected.content_hash, editContent);
      else await client.approve(selected.memory_id, selected.content_hash);
  ...
  const reject = async () => {
    ...
    setBusy("reject");
    try { await client.reject(selected.memory_id, rejectReason); setSelected(null); setRejectReason(""); await load(); }
    catch (reason) { if (reason instanceof ApiError) setError(reason); } finally { setBusy(""); }
  };
  ...
  const createCore = async () => {
    ...
    setBusy("create-core");
    try { await client.createCore(draft); setDraft(emptyDraft); await load(); }
    catch (reason) { if (reason instanceof ApiError) setError(reason); } finally { setBusy(""); }
  };

  const archive = async () => {
    if (!selected || busy || !window.confirm("归档后不再默认注入 Codex，但不会物理删除文件。")) return;
    setBusy("archive");
    try { await client.archive(selected.memory_id); setSelected(null); await load(); }
    catch (reason) { if (reason instanceof ApiError) setError(reason); } finally { setBusy(""); }
  };
  ...
  return <div className="loop-page">
    ...
    <section className="review-layout">
      ...
      <div className="loop-panel"><h2>审核详情</h2>{selected ? <><h3>{selected.title ?? selected.memory_id}</h3><p>来源 Session：{selected.source_session_id ?? "未知"}</p><p>来源 Message：{selected.source_message_id ?? "未知"}</p><p>AI 提议理由：{selected.proposal_reason ?? "未知"}</p><p>当前 Hash：{selected.content_hash ?? "未知"}</p><p>相似 Core Memory：{selected.similar_core?.map((item) => item.title ?? item.memory_id).join("；") || "无"}</p><p>影响 Agent：{selected.affected_agents?.join("；") || "未知"}</p><textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} /><div className="review-actions"><button className="button" disabled={!canApprove(selected.content_hash) || Boolean(busy)} onClick={() => void approve(false)}>{busy === "approve" ? "批准中…" : "批准"}</button><button disabled={!canApprove(selected.content_hash) || Boolean(busy)} onClick={() => void approve(true)}>{busy === "edit-approve" ? "保存中…" : "编辑后批准"}</button><input placeholder="拒绝理由（可选）" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} /><button disabled={Boolean(busy)} onClick={() => void reject()}>{busy === "reject" ? "拒绝中…" : "拒绝"}</button><button onClick={() => setSelected(null)}>稍后处理</button></div>{integrity && <div className="danger-note"><strong>长期记忆健康：{integrity.state}</strong><p>{integrityMessage(integrity.state)}</p><div className="review-actions"><button>查看详情</button><button onClick={() => setIntegrity(null)}>稍后处理</button><button disabled={Boolean(busy)} onClick={() => void archive()}>归档</button></div></div>}<p className="danger-note">归档后不再默认注入 Codex，但不会物理删除文件。</p></> : <p>选择候选记忆查看完整内容与来源。</p>}</div>
    </section>
