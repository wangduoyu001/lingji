...
export default function CodexWorkspacePage({ api, active, onOpenInspector }: Props) {
  ...
  const chooseProject = async () => {
    ...
    setBusy("resolve");
    try { const project = await client.resolve({ path: chosen }); setFilters((f) => ({ ...f, projectId: project.project_id, offset: 0 })); await load(); }
  ...
  const buildContext = async () => {
    ...
    setBusy("context");
    try { setContextPack(await client.context({ project_id: projectId, task: contextTask.trim(), max_chars: maxChars })); }
    catch (reason) { if (reason instanceof ApiError) setError(reason); } finally { setBusy(""); }
  };
