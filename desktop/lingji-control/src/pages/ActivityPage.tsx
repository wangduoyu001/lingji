...
function WorkCard({ item }: { item: WorkHistoryItem }) {
  ...
  return <article className="activity-card">
    <div className="activity-card-heading"><div><span className={`timeline-marker ${status === "failed" ? "failed" : status === "completed" || status === "success" ? "completed" : ""}`} /><strong>{shown(item.work?.title || summary.source, "一项灵机工作")}</strong></div><span className={`pill ${status === "failed" ? "failed" : status === "completed" || status === "success" ? "success" : "neutral"}`}>{phaseLabel(summary.phase, status)}</span></div>
    <p className="activity-card-result">结果：{shown(result || item.outcome?.summary)}</p>
    <div className="activity-card-meta"><span>时间：{time(summary.time || item.work?.updated_at || item.work?.created_at)}</span><span>来源：{shown(summary.source, "来源尚未获得")}</span><span>下一步：{shown(summary.next_actor, "暂时没有下一步")}</span></div>
