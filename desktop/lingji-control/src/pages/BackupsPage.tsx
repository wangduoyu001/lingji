  ...
  return (
    <div className="two-column">
      <Panel title="备份">
        ...
        <div className="list">{items.map((item) => <button className="list-button" key={item.backup_id} onClick={() => { setSelected(item); setConfirmation(""); }}><strong>{item.backup_id}</strong><small>{item.profile} · {bytes(item.archive_bytes)}</small></button>)}</div>
      </Panel>
      <Panel title="校验与隔离恢复">
