...
def observation_from_context_pack(
    ...
    *,
    ...
    """Adapt an existing ContextPack selection into the closed oracle shape."""
    if not isinstance(pack, Mapping):
        raise ResultSchemaError("ContextPack must be a mapping")
    by_fact = fixture.corpus_by_fact
    ...
    for section in sections:
        ...
        message_id = citation.get("message_id") or section.get("message_id")
        if isinstance(message_id, str) and message_id:
            by_message[message_id] = section
    for fact_id in fact_ids:
        record = by_fact.get(str(fact_id))
        if record is None:
            raise ResultSchemaError(f"unknown selected fact: {fact_id}")
        section = by_message.get(record.message_id)
        content_hash = record.content_hash
        if section is not None:
            citation = section.get("citation") if isinstance(section.get("citation"), Mapping) else {}
            candidate_hash = citation.get("content_hash") or section.get("content_hash")
            if isinstance(candidate_hash, str) and candidate_hash.strip():
                content_hash = candidate_hash.strip()
        identities.append({
            "fact_id": record.fact_id, "source_id": record.source_id,
            "conversation_id": record.conversation_id, "message_id": record.message_id,
            "content_hash": content_hash, "citation_id": record.citation_id,
        })
    answer_atoms = []
    for section in sections:
    ...
    return {
        ...
        "reason": (
            _text(reason_override, "reason_override")
...
class FrozenQuestionOracle:
    ...
    def __init__(self, fixture: FrozenFixtures):
    ...
    def evaluate(self, question: FrozenQuestion, *, gateway: Mapping[str, Any], mcp: Mapping[str, Any]) -> QuestionDiagnostic:
        if question.question_id not in {item.question_id for item in self.fixture.questions}:
        ...
        def inspect(value: Mapping[str, Any], role: str) -> None:
            ...
            sources = tuple(item.source_id for item in identities)
            messages = tuple(item.message_id for item in identities)
            for identity in identities:
                ...
                if identity.content_hash != expected.content_hash:
                    buckets.add("provenance"); failures.append(f"{role}_content_hash_mismatch")
            if facts != question.expected_fact_ids or sources != question.expected_source_ids or messages != question.expected_message_ids:
        ...
        false_positive = sum(
            1 for identity in gateway_value["identities"]
            if identity.fact_id in set(question.forbidden_fact_ids)
            or identity.source_id in set(question.disallowed_source_ids)
            or identity.message_id in set(question.disallowed_message_ids)
        )
        if false_positive:
        ...
        return QuestionDiagnostic(
            ...
            false_positive_count=false_positive, failure_buckets=_ordered_buckets(buckets),
    ...
    def run(
        ...
        *,
        ...
        for question in questions:
            diagnostic = checkpoint_store.load(question.question_id) if checkpoint_store else None
            if diagnostic is None:
                try:
                    ...
                    error_pack = {
                        "identities": [], "citations": [], "answer_atoms": [],
                        "used_chars": 0, "query_mode": question.mode, "as_of": question.as_of,
                        "reason": reason,
                    }
                    diagnostic = self.evaluate(question, gateway=error_pack, mcp=error_pack)
                if checkpoint_store:
...
class QuestionCheckpointStore:
    ...
    def __init__(self, root: str | Path, *, fixture_hashes: Mapping[str, str], run_id: str, code_commit: str):
    ...
    def save(self, result: QuestionDiagnostic) -> None:
        ...
        payload = {
            ...
            "question_id": result.question_id, "result": result.to_mapping(),
        }
    ...
    def load(self, question_id: str) -> QuestionDiagnostic | None:
        path = self._path(question_id)
        if not path.exists():
            return None
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            required = {"schema_version", "run_id", "code_commit", "fixture_hashes", "question_id", "result"}
