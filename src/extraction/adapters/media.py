...
class MediaExtractionAdapter(ExtractionAdapter):
    ...
    def extract(self, request: ExtractionRequest) -> ExtractionBatch:
        ...
        warnings: list[str] = []
        probe = self._probe(path, request.options, warnings)
        derivatives = self._derive(path, media_hash, media_kind, request.options, warnings)
        transcript = self._read_optional(
            request.payload.get("transcript") or request.options.get("transcript_path")
        )
        ocr_text = self._read_optional(
            request.payload.get("ocr_text") or request.options.get("ocr_path")
        )
        visual_notes = self._read_optional(
            request.payload.get("visual_notes") or request.options.get("visual_notes_path")
        )
    ...
    def _derive(
        ...
        if options.get("extract_keyframes") and media_kind == "video":
            ...
            max_frames = min(max(int(options.get("max_keyframes", 120)), 1), 500)
