...
class MemoryReviewService:
    ...
    def __init__(self, lifecycle, *, database=None, index_sync: Callable[[Path], Any] | None = None, state_db=None, evidence_store=None):
    ...
    def correct_core_memory(self, memory_id: str, *, content: str, expected_content_hash: str, owner_confirmed: bool, reason: str, title: str | None = None) -> dict[str, Any]:
        ...
        relationships = metadata.get("relationships") if isinstance(metadata.get("relationships"), dict) else {}
        category = source.parent.name or "General"
        # Preserve the complete evidence/provenance chain on the new version.
        # Only lifecycle/identity fields are regenerated by propose/approve;
        # source refs, relationships, confidence and validity remain auditable.
        preserved = dict(metadata)
        for key in {
            "id", "title", "memory_tier", "status", "review_status",
            "pin_to_context", "proposed_by", "created_at", "updated_at",
            "promoted_at", "approved_at", "approved_by", "approved_hash",
            "superseded_by", "rejected_at", "rejection_reason", "archived_at",
            "archive_reason", "invalidating_reason", "supersede_reason",
        }:
            preserved.pop(key, None)
        for key in ("evidence_refs", "source_refs", "source_id", "conversation_id", "message_id", "occurred_at", "content_hash"):
            if key not in preserved and relationships.get(key) not in (None, ""):
                preserved[key] = relationships[key]
        evidence_refs = relationships.get("evidence_refs") or relationships.get("source_refs") or preserved.get("evidence_refs") or preserved.get("source_refs") or []
        if evidence_refs:
            preserved["evidence_refs"] = self._list(evidence_refs)
    ...
    def archive_core_memory(self, memory_id: str, *, owner_confirmed: bool, reason: str, expected_content_hash: str = "") -> dict[str, Any]:
        ...
        self._sync(path)
        result = {"id": memory_id, "relative_path": self.layout.relative(path).as_posix(), "status": "archived", "archived_at": now, "reason": reason.strip()}
    ...
    def _copy_read_model_links(self, old_memory_id: str, new_memory_id: str) -> None:
        store = self.evidence_store
        if store is None or not hasattr(store, "memory_links") or not hasattr(store, "link_message_memory_batch"):
            return
        links = list(store.memory_links(old_memory_id) or [])
        refs = [
            {"message_id": str(link.get("message_id") or ""), "content_hash": link.get("content_hash")}
            for link in links
            if str(link.get("message_id") or "").strip()
        ]
        if refs:
            store.link_message_memory_batch(
                refs,
                new_memory_id,
                decision_id=f"owner-correction:{old_memory_id}:{new_memory_id}",
            )
