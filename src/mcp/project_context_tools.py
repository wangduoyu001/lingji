...
def register_project_context_tools(mcp, project_context_service, default_agent_resolver):
    async def lingji_build_context(project_id: str, task: str, session_id: str = "", max_chars: int | None = None, mode: str = "current", as_of: str | None = None):
        ...
        return project_context_service.build(agent_id=agent_id, project_id=project_id, query=task, session_id=session_id, max_chars=max_chars, allow_cross_project=False, mode=mode, as_of=as_of)
