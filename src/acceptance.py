...
class AcceptanceChecker:
    ...
    def _check_health(self) -> None:
        for item in StartupHealthChecker(self.settings, read_only=True).run().get("checks", []):
            details = {key: value for key, value in item.items() if key not in {"name", "status", "message"}}
            self._add(
    ...
    def _check_index(self) -> None:
        ...
        try:
            ...
            self._add("file_index", "ok", f"现有文件索引包含 {len(entries)} 条记录", path=str(path), entries=len(entries))
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
            self._add("file_index", "error", f"读取文件索引失败：{exc}", path=str(path))

    def _check_databases(self) -> None:
        for name, path in (("state_db", self.settings.state_db_path), ("memory_db", self.settings.memory_db_path)):
            if not path.exists():
    ...
    def _check_runtime_settings(self) -> None:
        ...
        try:
            ...
            self._add("runtime_settings", "ok", f"发现 {len(overrides)} 项用户覆盖", path=str(path), override_keys=sorted(overrides))
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
    ...
    def _check_chatgpt_export(self) -> None:
        ...
        if path.is_dir():
            ...
            self._add("chatgpt_export", "ok" if recognized else "warning", "ChatGPT 解压目录可识别" if recognized else "未发现标准文件", path=str(path), sample_files=names)
            return
        if path.suffix.lower() == ".json":
            self._add("chatgpt_export", "ok", "ChatGPT JSON 文件可访问", path=str(path), bytes=path.stat().st_size, sha256=self._sha256(path) if self.hash_inputs else None)
            return
        ...
        try:
            ...
            self._add("chatgpt_export", status, message, path=str(path), members=len(members), uncompressed_bytes=total, archive_bytes=path.stat().st_size, deep_zip_check=self.deep_zip_check, sha256=self._sha256(path) if self.hash_inputs else None)
    ...
    def _check_media(self) -> None:
        ...
        if not ffprobe:
            ...
            return
        try:
            result = subprocess.run([ffprobe, "-v", "error", "-print_format", "json", "-show_format", "-show_streams", str(path)], check=True, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=60)
            probe = json.loads(result.stdout or "{}")
            self._add("media_sample", "ok", "样例媒体 FFprobe 读取成功", path=str(path), bytes=path.stat().st_size, sha256=self._sha256(path) if self.hash_inputs else None, format=(probe.get("format") or {}).get("format_name"), duration=(probe.get("format") or {}).get("duration"), streams=len(probe.get("streams") or []))
        except (subprocess.SubprocessError, json.JSONDecodeError, OSError) as exc:
            self._add("media_sample", "error", f"样例媒体检查失败：{exc}", path=str(path))

    def _capture_inputs(self) -> dict[str, Any]:
        return {
            "vault": self._fingerprint(self.settings.vault_path, markdown_only=True),
            "state_db": self._fingerprint(self.settings.state_db_path),
            "memory_db": self._fingerprint(self.settings.memory_db_path),
    ...
    def _fingerprint(self, path: Path, *, markdown_only: bool = False) -> dict[str, Any]:
        if not path.exists():
            return {"exists": False, "path": str(path)}
        if path.is_file():
            stat = path.stat()
            return {"exists": True, "kind": "file", "path": str(path), "bytes": stat.st_size, "mtime_ns": stat.st_mtime_ns, "sha256": self._sha256(path) if self.hash_inputs else None}
        digest = hashlib.sha256()
        count = total = 0
        iterator: Iterable[Path] = path.rglob("*.md") if markdown_only else path.rglob("*")
        files = sorted((item for item in iterator if item.is_file() and not item.is_symlink()), key=lambda item: item.as_posix())
        for item in files:
            stat = item.stat()
            count += 1
            total += stat.st_size
        ...
        return {"exists": True, "kind": "directory", "path": str(path), "files": count, "bytes": total, "digest": digest.hexdigest(), "content_hashed": self.hash_inputs}
