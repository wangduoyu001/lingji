...
class AssistantImportPlanner:
    """Build an owner-facing import plan without reading source bodies."""
        ...
        return {
            ...
            "sources": [
                self._source(
                    ...
                    guide="灵机会先自动找官方导出包，只在读取正文前向你确认一次。",
                ),
                self._source(
                    source_id="codex",
                    label="Codex 工作记录",
                    ...
                    guide="结构化 Work Report 可以直接进入处理队列；原始 Session 不会被静默读取。",
                ),
                self._source(
    ...
    def _scan_candidate_paths(self) -> list[tuple[ImportCandidate, Path]]:
        ...
        for root in self._candidate_roots():
            ...
            if len(result) >= _MAX_CANDIDATES:
                break
        result.sort(key=lambda item: item[0].modified_at or "", reverse=True)
        return result
    ...
    def _classify(path: Path) -> tuple[str, str, str] | None:
        ...
        if suffix in {".zip", ".json"} and (
            "chatgpt" in name or name == "conversations.json" or ("openai" in name and "export" in name)
        ):
            return "chatgpt", "chatgpt_export", "chatgpt_export"
        if suffix == ".json" and "codex" in name and ("report" in name or "work" in name):
