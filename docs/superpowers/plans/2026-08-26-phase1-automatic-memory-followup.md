...
**Interfaces:**
- `ExpectedImportedRow` is created from the Generic History adapter's parsed output before persistence and contains the adapter-produced external source/conversation/message IDs, order, role, content hash and timestamp. `ImportedEvidenceAudit.from_read_model(read_model, expected_rows: Sequence[ExpectedImportedRow]) -> ImportedEvidenceAudit` obtains all actual persisted rows from the existing `SourceReadModel` and computes exact total, extra, missing, duplicate, ordered external message ID, role, sequence, content-hash, source and conversation comparisons. It must not compare raw fixture IDs directly to generated database IDs and must not compute metrics from a pre-deduplicated expected map.
...
- The pre-query identity map is built once from adapter-produced external message IDs/content hashes to corpus fact/citation labels. Those labels are never written into imported rows. Selection walks the ordered Gateway output with one constant rule, extracts only stable returned message IDs/content hashes, then maps them to evaluator labels; modifying a question's expected/forbidden/citation IDs cannot change selected evidence.
