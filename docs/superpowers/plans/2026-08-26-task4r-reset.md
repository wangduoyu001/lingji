...
- Each ingestion item has the exact read-only fields required by Task 2: `source_id`, `source_external_id`, `conversation_id`, `conversation_external_id`, `message_id`, `message_external_id`, `ingestion_batch_id`, `ingestion_ordinal`, `sequence`, `role`, `occurred_at`, `content_hash`. The query obtains external parents with explicit joins and never returns message content, metadata, privacy or credentials through this audit API.
