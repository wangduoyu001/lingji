...
**Breaker and owner-authorized recovery (2026-08-27):** Repair Round 2 review at product/docs head `3227a279990e3977b73a8f0ba7463aeed13deeb2` returned Spec FAIL / Quality Needs fixes because the real recovery matrix was incomplete, the compatibility event path bypassed redaction, and non-finite ordinary payloads were persistable. The owner authorized the separate one-shot safety plan `docs/superpowers/plans/2026-08-27-promotion-safety-closeout.md`. Its implementation closed the two serialization blockers, but independent review at `f414a4f09cb92f0c30bc5124e34112263bbce84f` rejected the claimed twelve-case durable proof (0 Critical, 2 Important). Per the no-loop rule, there is no further recovery repair round. `docs/superpowers/plans/2026-08-27-owner-review-quarantine.md` now disables silent automatic activation while preserving archival, retrieval, candidate generation and explicit owner approval. After that bounded quarantine passes review, this plan resumes at Task 1.
...
- [ ] **Step 1: Write RED API tests for work history pagination, friendly event summaries and pending-action resolution.** Assert stable `work_id`/`action_id`, restart persistence and no mismatch between current/history/pending projections.
