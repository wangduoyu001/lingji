...
## 2026-07-29 · 验收治理 · 建立统一 Codex 验收权威
...
- [ ] `python scripts/check_acceptance_sync.py`：产品相关文件变化时必须同步修改 `docs/ACCEPTANCE/`。
- [ ] `python -m pytest -q tests/test_acceptance_sync.py`：覆盖无代码变化、代码未同步文档、代码已同步文档、工作区模式和 CI 基线模式。
- [ ] GitHub Workflow `acceptance-doc-sync`：PR 和 feature/master push 上执行同步门禁。

### 新增或修改的真机验收

- [ ] Codex 拉取仓库后读取 `AGENTS.md`，能够定位本目录和通用验收指令。
- [ ] 使用当前变更记录生成对应验收清单，未依赖聊天历史。
- [ ] 验收结束删除临时 Artifact、日志、截图、fixture 和配置临时副本。

### 主人肉眼确认

- [ ] 仓库中的验收入口和规则对主人而言清晰，不再需要从长对话中寻找指令。

### 回归项

- [ ] 不允许代码变更后遗漏验收标准更新。
- [ ] 不允许为了补报告移动已打包的产品 Head。
- [ ] 不允许长期堆积重复安装包、日志、截图和配置备份。
...
- 报告分支：`docs/acceptance-governance`
