...
> Current bounded candidate product/tests head: `6ea11e491868e227a0c454e87f73ebd92a99b788` (owner UI live repair)
...
> Current engineering gate: `OWNER_UI_LIVE_REPAIR_MAC_6EA11E4 — MAC OWNER EXPERIENCE PENDING`
...
`OWNER_UI_LIVE_REPAIR_MAC_6EA11E4` 已激活为唯一 `ACTIVE` 本机任务，执行模式为
`MACOS_OWNER_UI_EXPERIENCE_ONLY`，精确产品 SHA 为
`6ea11e491868e227a0c454e87f73ebd92a99b788`，候选标签为
`OWNER_UI_LIVE_REPAIR_CANDIDATE`。本轮只做隔离 macOS arm64 Tauri 候选构建、整包安装备份、
...
Task8E Safe Polling Fallback Repair Round 2 已完成代码与合成 fixture 验证：Darwin/macOS 在未显式配置时使用 `periodic_reconciliation`，不启动不可靠的 `watchfiles` event watcher；Windows 与其他平台默认保持 event watcher，显式配置仍可覆盖。未知/畸形平台值 fail-closed 到 periodic。现有启动增量扫描、15 分钟默认 reconciliation、每日 integrity、手动扫描、授权/revoke 和 pause/resume/restart 生命周期保持在同一 scheduler/registry。Runtime/API 暴露真实 reconciliation interval 与最大变化发现延迟，且仅对 finite-positive interval 给出数字/分钟文案，Desktop 根据字段显示，字段缺失或非法时显示“尚未获得”。产品/测试提交为 `6862c46`、`eff22b4`、`cf09946102d89ddf67f3f69bae79f7bd45180dfe`、`be73008407b3540eeb8bbcbd040ddc69faf4adc7`，报告位于 `.superpowers/sdd/2026-08-29-task8e-safe-polling-fallback/task-report.md`。本 fallback 不满足 30 秒事件 SLA，只保证按配置周期自动核对（默认最迟 15 分钟）；Phase 1 自动接管门禁仍为 `BLOCKED`。未执行 live 8766/8767、打包、安装、Artifact、Production/Vault 或主人验收。

Task8E Owner-facing plain UI repair 已按主人反馈“仍然看不懂”完成代码与合成 Playwright fixture 验证：普通首页现在直接显示灵机是否正常、正在记住的来源、最近一次检查及时间、以及是否需要主人处理；来源/活动/待办页面移除技术字段堆叠，source/work/internal ID 与 JSON 仅在折叠详情。定期检查文案使用 API 真实间隔（默认显示“打开灵机时会检查，之后每15分钟自动检查一次”），缺失/非法间隔显示“检查时间尚未获得”；Managed Obsidian 与 Claude 均提供完整中文文案。授权、现在检查、停止记忆、再次检查、查看这次检查与待办解决仍接入原有 API。Owner fixture 覆盖 0 来源、1 来源、失败/重试和待办解决；未执行 live、打包、安装、Artifact、Production/Vault 或主人验收，等待根代理授权的 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 1 已按独立 review 修复 I1–I5/M1：`work:null` 显示“目前空闲”；真实 completed/running/failed summary/scan 状态不再误报，缺计数不补默认 0；pending stale/error 显示“待办状态暂时无法确认，正在重试”；Claude-only/零可用来源显示“暂时没有可连接的记录来源。”；Codex/ChatGPT/generic/unknown 来源统一中文；侧栏 development/commit/version/raw last_error 与路径移入折叠运行详情。产品/测试提交 `6a84c49f7505e3226ff53e077284446d39635a57`；rendered E2E、23-script smoke、build、backend focused、compile/diff/sync/handoff 均需以本轮最终证据为准。仍未执行 live、打包、安装、Artifact、Production/Vault 或主人验收，等待 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 2 已按独立复审修复 I1–I3：Current Work 的 queued/accepted/running/retrying/pending/completed/success/failed/cancelled 全部有中文状态；Claude `consent_required + 空路径` 不再计入可连接来源，真实有路径/已授权来源保持可用；scan detail 的非终态 model 默认 0 不再伪装为真实计数，completed 的明确 0 仍保留。产品/测试提交 `f2f48430a13d8606b33d16eddff385fdcb580d5c`；rendered E2E、23-script smoke、build、backend focused `56 passed`、compile/diff/sync/handoff 均已通过。未执行 live、打包、安装、Artifact、Production/Vault 或主人验收，等待 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 3 已按最终复审唯一 Important 收口 completed scan detail 计数证据：Desktop adapter 不再把 ScanRun/legacy model 默认 0 当作真实结果；缺字段及无明确存在标记显示“检查结果尚未获得”，明确真实 0 与正数保留，Home 摘要与详情共用同一语义。rendered fixture 覆盖缺字段、legacy/default 0、明确 0、明确正数；`npm run test:e2e:memory`、23-script smoke、build、backend focused `56 passed, 1 warning`、compile/diff/sync/handoff 均通过。产品/测试与 docs/report 将分开提交；未执行 live、打包、安装、Artifact、Production/Vault 或主人验收，仍等待 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 4 已把 scan count 的“确实测到”证据贯通正式链路：StateDB 对 `queued_count`/`reused_count` 使用可空字段并兼容旧库，旧记录保持未知；Snapshot/调度完成时原子写入真实新增/复用数，空扫描的真实 0 可见，暂停/失败/崩溃/legacy 未测量保持“尚未获得”。`ScanRun`、Local Control API 的列表/摘要/详情/动作统一输出同一 DTO presence 语义，兼容默认 0 不再冒充真实结果，同秒扫描排序稳定。Owner rendered E2E、23-script smoke、build、后端 focused 与 Task2–5 相关回归已通过；宽仓库回归仍有既有质量评测/环境基线失败，未改变这些范围外问题。产品/测试提交为 `33b730ec8de1b7b545da7c5eecdb70cdcf67e30e`，尚未打包/安装/真实主人验收；下一步是由根代理基于该精确提交生成新 Mac Artifact 并让主人重新观察，保持 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 5 已收口 Round4 终审的两个重要契约问题：scheduler 不再把 legacy/未测量 `ScanRun` 的计数缺失值变成伪造 `0`，真实完成且实际测量的 0/正数仍保留；Local Control API 的 action、list、summary、detail 统一投影稳定 `work_id`，并保留过期/不支持来源的真实 `complete/errors/next_action` 结果。产品/测试提交为 `ca6b8ea40cf6d95974639af213ca6ee41e08bdee`。直接后端 `69 passed, 1 warning`，Round5 专项 `17 passed, 1 warning`，Task2–5 affected `184 passed, 3 warnings`，packaged contract `2 passed, 1 warning`（291.19 秒），Desktop rendered E2E、23-script smoke、build（92 modules）、compileall、diff-check 均通过。未启动/安装 live App，未访问 Acceptance/Production/Vault/主人数据；主人观察与最终 Mac Artifact 仍待根代理执行，状态保持 `READY_FOR_OWNER_EXPERIENCE`。

Task8E Owner-facing plain UI repair Round 6 已修复 Round5 终审遗漏的早退语义：`ReconciliationReport` 默认不宣称计数已测量；暂停、unsupported/degraded/expired、已有扫描占用和异常路径均保留未知计数，内部进度仍使用局部安全回退。正式 report、reconciliation 事件与认证 scan action 现在提供可执行的 `next_action`，分别覆盖恢复/重试、官方不支持、重新授权、修复来源、等待现有检查和手动重试。产品/测试提交为 `58e12ca37b321075445c49057c54851f55539baf`；后端 focused `65 passed, 1 warning`，Task2–5 affected `229 passed, 3 warnings`，Desktop E2E、23-script smoke、build（92 modules）、compileall、diff-check、acceptance-sync、local-handoff 均通过。未启动/安装 live App，未访问 Acceptance/Production/Vault/主人数据；最终 Mac Artifact 与主人观察仍待根代理，状态保持 `READY_FOR_OWNER_EXPERIENCE`。
