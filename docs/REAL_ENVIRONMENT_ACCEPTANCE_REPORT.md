...
- 权威验收条件：代码 CI 全绿，并在主人电脑对真实 Vault、ChatGPT 导出和样例媒体运行一次
...
1. Python sqlite3 URI 和只读模式：
   ...
   - 采用 `file:...?...mode=ro`，避免验收过程创建或修改数据库。
2. Python zipfile 完整性检查：
   - https://docs.python.org/3.12/library/zipfile.html#zipfile.ZipFile.testzip
   - `testzip()` 读取所有成员并检查 CRC，返回首个损坏成员。
3. FFprobe 官方文档：
   - https://ffmpeg.org/ffprobe.html
   - 使用 `-show_format -show_streams -print_format json` 只读获取媒体参数。
4. Ollama 官方 API：
...
- `tests/test_acceptance_read_only.py`
  - 输入不变、只读 SQLite、ZIP CRC 和报告目录测试
...
- `src/health.py`
  ...
  - 不创建目录、不写探针、SQLite 只读、Ollama 模型列表
- `scripts/acceptance_check.py`
  - 变成薄 CLI，复用统一 Service
...
3. 验收期间其他程序修改文件会导致 `inputs_unchanged=false`，这是正确的失败而不是误报隐藏。
4. 活跃 SQLite 进程可能在验收期间写入 WAL，系统会报告输入变化。
5. FFprobe 只验证媒体可读和结构，不执行转写、OCR 或画面理解。

## 已知限制

- GitHub Actions 无法访问主人电脑的 `E:\obsidian\本地知识库`。
- 代码通过 CI 后，P0-B 仍需主人电脑真实运行才能标记 `ACCEPTED`。
- 当前验收入口通过顶层 UI 模式切换提供；P1-0 UI 模块化后再并入正式侧栏路由。
- 报告路径当前以文本显示，Tauri 的“打开目录”按钮在桌面打包阶段实现。

## 回滚

本模块没有数据迁移。回滚分支提交即可；删除派生验收报告不会影响 Vault、Raw、Backup、索引或正式知识。
