...
def start_packaged_extraction_worker(
    ...
    *,
    ...
    """Start the durable capture worker owned by the packaged Sidecar.

    The control API accepts governed imports by queueing them.  A packaged
    runtime must also own a worker for that same durable queue; otherwise a
    successful one-action authorization is left permanently in ``queued``.
    Imports stay lazy so configuration has been applied before Settings and
    the extraction stack are created.
    """
